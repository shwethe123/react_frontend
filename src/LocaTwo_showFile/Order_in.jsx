            ...
            Array.isArray(orderIn) && orderIn.length > 0 ? (orderIn.map((orderIn_data, index) => (
                <Link to={`/OrderIn_loc2/Edit/${orderIn_data._id}`} key={orderIn_data._id}>
                    <p className={`
                        ...
                        ${orderIn_data.condition === 'ခွင့်ရက်ရှည်' ? 'bg-green-700 border-spacing-4 border-red-300... text-white' : ''} 
                        ${orderIn_data.condition === 'ခွင့်မဲ့' ? 'bg-red-500 text-white' : ''} 
                        ${orderIn_data.condition === 'ဖိုင်းအပြစ်ပေး' ? 'bg-red-400 border-l-4 border-red-600 ... text-white' : ''}
                        ${orderIn_data.condition === 'အလုပ်ထွက်မည့်သူ' ? 'bg-gray-700 border-l-4 border-black ... text-white' : ''}
                        ${orderIn_data.condition === 'အလုပ်ထွက်သူ' ? 'bg-black text-white' : ''} '} 
