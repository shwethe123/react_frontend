            ...
            Array.isArray(saleOut) && saleOut.length > 0 ? (saleOut.map((saleOut_data, index) => (
                <Link to={`/SaleOut2/Edit/${saleOut_data._id}`} key={saleOut_data._id}>
                    <p className={`
                        ...
                        ${saleOut_data.condition === 'ခွင့်ရက်ရှည်' ? 'bg-green-700 border-spacing-4 border-red-300... text-white' : ''} 
                        ${saleOut_data.condition === 'ခွင့်မဲ့' ? 'bg-red-500 text-white' : ''} 
                        ${saleOut_data.condition === 'ဖိုင်းအပြစ်ပေး' ? 'bg-red-400 border-l-4 border-red-600 ... text-white' : ''}
                        ${saleOut_data.condition === 'အလုပ်ထွက်မည့်သူ' ? 'bg-gray-700 border-l-4 border-black ... text-white' : ''}
                        ${saleOut_data.condition === 'အလုပ်ထွက်သူ' ? 'bg-black text-white' : ''} '} 
