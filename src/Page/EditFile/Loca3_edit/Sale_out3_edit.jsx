...
export default function Sale_out3_edit() {
    ...
    useEffect(() => {
        if (id) {
            const fetch_api = async () => {
                try {
                    const response = await axios.get(`https://dashboard-yfuz.onrender.com/api/saleOut_three/${id}`);
    ...
    const UpdatePost = async (e) => {
        try {
            ...
            let res;
            if (id) {
                res = await axios.patch(`https://dashboard-yfuz.onrender.com/api/saleOut_three/${id}`, addPost);
            } else {
                res = await axios.post(`https://dashboard-yfuz.onrender.com/api/saleOut_three`, addPost);
                if (res.status === 200) {
                    navigate('/LocaOne');
                }
            };
